...
import { OrderStatus } from "data/types/orders.types";
  ...
  test.describe("[Update Order Status Positive]", () => {
    test(
      ...
      async ({ ordersApiService, ordersApi }) => {
        ...
        const updatedStatus = await ordersApi.updateOrderStatus(draftWithDelivery._id, ORDER_STATUS.IN_PROGRESS, token);
    ...
    test(
      ...
      async ({ ordersApiService, ordersApi }) => {
        ...
        const updatedStatus = await ordersApi.updateOrderStatus(orderInProcess._id, ORDER_STATUS.CANCELED, token);
        validateResponse(updatedStatus, {
          status: STATUS_CODES.OK,
          schema: orderResponseSchema,
          IsSuccess: true,
    ...
    test(
      ...
      async ({ ordersApiService, ordersApi }) => {
        ...
        const updatedStatus = await ordersApi.updateOrderStatus(canceledOrderInProcess._id, ORDER_STATUS.DRAFT, token);
        validateResponse(updatedStatus, {
          status: STATUS_CODES.OK,
          schema: orderResponseSchema,
          IsSuccess: true,
  ...
  test.describe("[Update Order Status Negative]", () => {
    ...
    test(
      ...
      async ({ ordersApiService, ordersApi }) => {
        ...
        const updatedStatus = await ordersApi.updateOrderStatus(orderInProcess._id, ORDER_STATUS.RECEIVED, token);
        validateResponse(updatedStatus, {
          status: STATUS_CODES.BAD_REQUEST,
          IsSuccess: false,
          ErrorMessage: ERROR_MESSAGES.INVALID_ORDER_STATUS,
    ...
    test(
      ...
      async ({ ordersApiService, ordersApi }) => {
        ...
        const updatedStatus = await ordersApi.updateOrderStatus(draft._id, ORDER_STATUS.IN_PROGRESS, token + "5");
        validateResponse(updatedStatus, {
          status: STATUS_CODES.UNAUTHORIZED,
          IsSuccess: false,
